...
class DeviceFailureZoneBridge:
    ...
    def execute(self, sweep: SweepResult, tool_state: ToolState) -> DeviceFailureZoneResult:
        ...
        activated = np.minimum(failure.active_thickness_m, patch.H_resting_m)
        ...
        if failure.applicability_status in {"OUTSIDE_FEE_DOMAIN", "PARTIAL_OUTSIDE_FEE_DOMAIN"}:
            activated = np.maximum(
                activated, np.minimum(intersection.penetration_depth_m, patch.H_resting_m)
            )
            mode = "GEOMETRIC_SWEEP_ONLY_OUTSIDE_FEE_FORCE_DOMAIN"
        elif not np.any(activated > 0.0):
            mode = "NONE"
        volume = local_integrator.integrate(activated)
        resting = patch.H_resting_m - activated
        mobile = patch.H_mobile_m + activated
        # Failure activation transfers mass only.  The existing Mobile backend
        # supplies tool momentum on a compact true-contact neighborhood.
        momentum = np.asarray(patch.mobile_momentum_m2_s, dtype=np.float64).copy()
        start = perf_counter()
        self.state.capture_surface_for_dirty_tracking()
        self.state.apply_host_patch("resting", bbox, resting, reason="failure_zone_activation")
        self.state.apply_host_patch("mobile", bbox, mobile, reason="failure_zone_activation")
        self.state.apply_host_patch("momentum_x", bbox, momentum[..., 0], reason="failure_zone_activation")
        self.state.apply_host_patch("momentum_y", bbox, momentum[..., 1], reason="failure_zone_activation")

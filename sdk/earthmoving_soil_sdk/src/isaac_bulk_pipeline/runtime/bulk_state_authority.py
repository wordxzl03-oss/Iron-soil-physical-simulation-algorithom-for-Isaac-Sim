...
def _state_kernels(wp: Any) -> tuple[Any, ...]:
    ...
    def sum_reservoirs(
        ...
        item = wp.tid()
        weight = weights[item]
        wp.atomic_add(summary, 0, resting[item] * weight)
...
class DeviceBulkState:
    ...
    def apply_host_patch(
        ...
        *,
        ...
        if field not in {"resting", "mobile", "momentum_x", "momentum_y"}:
    ...
    def explicit_host_view(self, *, source: str) -> HostBulkStateView:
        ...
        resting = self._download("resting", full_field=True).reshape(self.shape)
    ...
    def restore_checkpoint_fields(
        ...
        *,
        ...
        float_fields = {
            "resting", "mobile", "momentum_x", "momentum_y",
            "track_rut",
    ...
    def assert_consistent(self) -> None:
        ...
        required = {
            "resting", "mobile", "momentum_x", "momentum_y", "initial_resting",
    ...
    def reset(self) -> None:
        ...
        wp = self.runtime.wp
        wp.copy(self.runtime.arrays["resting"], self.runtime.arrays["initial_resting"])
        for name in (

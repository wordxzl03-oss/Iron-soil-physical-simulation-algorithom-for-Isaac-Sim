...
def _kernels(wp: Any) -> tuple[Any, ...]:
    ...
    def initialize(
        ...
        layer_depth = wp.min(resting[index], mobilization_depth)
        ...
        active = resting[index] > dry_tolerance and start_margin > wp.float64(0.0)
        unstable[index] = 0
        parent[index] = -1
        if active:
            unstable[index] = 1
        ...
        # The latch prevents the same mobilization tranche from being counted
        # repeatedly while it is still locally owned by Mobile.  It must not
        # permanently shield newly exposed Resting material from Y_start.
        # Once Mobile has left the cell, the next physically yielded tranche
        # is eligible even if the face itself remains above Y_start.  A cell
        # which still contains Mobile retains the hysteretic Y_stop rule.
        if (
            release_settled_latches != 0
            and speed <= activity_speed
            and (
                mobile[index] <= dry_tolerance
    ...
    def aggregate_components(
        ...
        if latch[index] == 0:
            transfer = wp.min(resting[index], mobilization_depth * severity[index])
    ...
    def mobilize(
        ...
        gy = gradient_y[index]
        magnitude = wp.sqrt(gx * gx + gy * gy)
        inv_normal = wp.float64(1.0) / wp.sqrt(wp.float64(1.0) + magnitude * magnitude)
        sin_slope = magnitude * inv_normal
        cos_slope = inv_normal
        acceleration = wp.float64(9.81) * wp.max(
            sin_slope - mobile_friction * cos_slope, wp.float64(0.0)
        )
        speed = velocity_efficiency * wp.sqrt(
            wp.float64(2.0) * acceleration * gravity_length
        )
        vx = wp.float64(0.0)
        vy = wp.float64(0.0)
        if magnitude > wp.float64(1.0e-12):
            vx = -gx / magnitude * speed
            vy = -gy / magnitude * speed
...
class DeviceLargeAvalancheBridge:
    ...
    def observe_and_maybe_mobilize(
        ...
        rt.launch(
            kernels[3], dim=size,
            inputs=[
                rt.arrays["avalanche_parent"], rt.arrays["resting"],
        ...
        if ready:
            ...
            rt.launch(
                ...
                inputs=[
                    ...
                    rt.arrays["avalanche_latch"], rt.arrays["avalanche_gradient_x"],

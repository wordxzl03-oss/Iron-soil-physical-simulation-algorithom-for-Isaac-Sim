...
@dataclass(frozen=True)
class AirborneParcelFootprint:
    """Finite horizontal landing support carried by a reduced-order parcel.

    The two extents describe a compact rectangle at impact.  Its lateral axis
    is stored in world coordinates so a rotated terrain can project it into
    its own XY plane without assuming world/terrain alignment.
    """
    ...
    provenance: str
    mass_profile: str = "UNIFORM_RECTANGLE"

    def __post_init__(self) -> None:
        axis = _readonly_array(
            self.lateral_axis_world_m,
            name="parcel.footprint.lateral_axis_world_m",
            shape=(3,),
        )
        norm = float(np.linalg.norm(axis))
        if norm <= 1.0e-12:
            raise ValueError("[MaterialParcel] footprint lateral axis is degenerate")
        lateral = _finite(
            self.lateral_extent_m, name="parcel.footprint.lateral_extent_m"
        )
        longitudinal = _finite(
            self.longitudinal_extent_m,
            name="parcel.footprint.longitudinal_extent_m",
        )
        if lateral <= 0.0 or longitudinal <= 0.0:
            raise ValueError("[MaterialParcel] footprint extents must be positive")
        if not isinstance(self.provenance, str) or self.provenance not in {
            "DIRECT_GEOMETRIC",
            "CONSERVATION_BASED_ENGINEERING_CLOSURE",
        }:
            raise ValueError("[MaterialParcel] invalid footprint provenance")
        if self.mass_profile not in {
            "UNIFORM_RECTANGLE",
            "ELLIPTIC_CONE_STOP_ANGLE_CLOSURE",
        }:
            raise ValueError("[MaterialParcel] invalid footprint mass profile")
        object.__setattr__(self, "lateral_axis_world_m", _readonly_array(axis / norm, name="parcel.footprint.normalized_lateral_axis", shape=(3,)))
        object.__setattr__(self, "lateral_extent_m", lateral)
        object.__setattr__(self, "longitudinal_extent_m", longitudinal)

    @property
    def area_m2(self) -> float:
        return float(self.lateral_extent_m * self.longitudinal_extent_m)
...
class MaterialParcel:
    ...
    timestamp_s: float
    source: str
    footprint: AirborneParcelFootprint | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.footprint is not None and not isinstance(
            self.footprint, AirborneParcelFootprint
        ):
            raise TypeError("[MaterialParcel] footprint must be AirborneParcelFootprint")
...
@dataclass(frozen=True)
class TerrainState:
    """Host-reference terrain state with one physical geometry authority.

    ``H_resting_m`` is retained as a source-compatible name for ``b_eff``;
    it is not a bookkeeping ownership field.  Physical resting depth is the
    derived ``b_eff-z_base`` quantity.
    """
    ...
    def __post_init__(self) -> None:
        ...
        base = (
            np.zeros_like(resting)
            if self.z_base_m is None
            else _readonly_array(
                self.z_base_m, name="z_base_m", shape=resting.shape,
                nonnegative=True,
            )
        )
        if np.any(resting < base - 1.0e-14):
            raise ValueError("[TerrainState] b_eff must not lie below z_base")
    ...
    def H_free_m(self) -> np.ndarray:
        return self.b_eff_m + self.mobile_height_m
    ...
    def snapshot(self) -> "TerrainState":
        ...
        return TerrainState(
            ...
            z_base_m=np.array(self.z_base_m, copy=True),

...
class LargeAvalancheTransitionController:
    ...
    def __init__(self, config: LargeAvalancheTransitionConfig | None = None) -> None:
        ...
        self._mobilized_owned_surface_m: np.ndarray | None = None
        self._mobilized_export_baseline_m3: np.ndarray | None = None
    ...
    def reset(self) -> None:
        ...
        self._mobilized_owned_surface_m = None
        self._mobilized_export_baseline_m3 = None
    ...
    def observe_and_maybe_mobilize(
        ...
        *,
        ...
        conservative_export_cumulative_m3: np.ndarray | None = None,
        ...
        diagnostics, grad_x, grad_y, slope_rad = self._diagnose(
            ...
            conservative_export_cumulative_m3=conservative_export_cumulative_m3,
        )
        ...
        if diagnostics.transition_ready:
            ...
            self._mobilized_owned_surface_m[activated] = (
                resting[activated] + mobile[activated]
            )
            if conservative_export_cumulative_m3 is None:
                self._mobilized_export_baseline_m3[activated] = 0.0
            else:
                exports = np.asarray(
                    conservative_export_cumulative_m3, dtype=np.float64
                )
                if exports.shape != grid.shape or not np.all(np.isfinite(exports)):
                    raise ValueError(
                        "[LargeAvalanche] conservative export evidence shape/finite check failed"
                    )
                self._mobilized_export_baseline_m3[activated] = exports[activated]
    ...
    def diagnose(
        ...
        diagnostics, _, _, _ = self._diagnose(
            ...
            conservative_export_cumulative_m3=None,
        )
        return diagnostics
    ...
    def _diagnose(
        ...
        *,
        ...
        conservative_export_cumulative_m3: np.ndarray | None,
        ...
        if self._mobilized_latch_mask is not None and release_settled_latches:
            ...
            if self._mobilized_owned_surface_m is None:
                self._mobilized_owned_surface_m = np.array(surface, copy=True)
            resolved_export = np.zeros(grid.shape, dtype=bool)
            if conservative_export_cumulative_m3 is not None:
                exports = np.asarray(
                    conservative_export_cumulative_m3, dtype=np.float64
                )
                if (
                    exports.shape != grid.shape
                    or not np.all(np.isfinite(exports))
                    or np.any(exports < 0.0)
                ):
                    raise ValueError(
                        "[LargeAvalanche] conservative export evidence must be finite/non-negative"
                    )
                if self._mobilized_export_baseline_m3 is None:
                    self._mobilized_export_baseline_m3 = np.array(exports, copy=True)
                resolved_export = (

...
def _kernels(wp: Any) -> Any:
    ...
    def apply_sources(
        ...
        source_energy: wp.array(dtype=wp.float64),
    ):
        index = wp.tid()
        row = index // cols
        col = index - row * cols
        ...
        if row + 1 < rows:
            up = index + cols
        # H_free is authoritative. Resting-only gradients contain artificial
        # ownership interfaces produced by conservative R2M/M2R relabeling.
        ...
        wp.atomic_add(
            source_energy, 0,
            wp.float64(0.5) * mass
            * (vx_g * vx_g + vy_g * vy_g - vx0 * vx0 - vy0 * vy0),
        )
        ...
        wp.atomic_add(
            source_energy, 1,
            wp.float64(0.5) * mass
            * (vx_f * vx_f + vy_f * vy_f - vx_g * vx_g - vy_g * vy_g),
        )
        vx = vx_f
        vy = vy_f
        if forcing[index] != 0:
        ...
        wp.atomic_add(
            source_energy, 2,
            wp.float64(0.5) * mass
            * (vx * vx + vy * vy - vx_f * vx_f - vy_f * vy_f),
        )
        velocity_x[index] = vx
        velocity_y[index] = vy
    ...
    def apply_fluxes(
        ...
        wp.atomic_add(transport_crossings, 1, amount * velocity_x[donor])
        wp.atomic_add(transport_crossings, 2, amount * velocity_y[donor])
    ...
    def summarize(
        ...
        if h > 1.0e-12:
            ...
            wp.atomic_add(
                summary, 4,
                wp.float64(0.5) * density * h * weights[index]
                * (vx * vx + vy * vy),
            )
...
class WarpMobileStep:
    ...
    gravity_pressure_work_j: float
    basal_friction_work_j: float
    tool_work_j: float
    transport_numerical_energy_residual_j: float
    donor_export_m3: float
...
class WarpMobileLayerSolver:
    ...
    def __init__(
        ...
        *,
        ...
        self._forcing_array_name = "forcing"
    ...
    def _initialize_workspace(self) -> None:
        ...
        if "mobile_export_cumulative" not in self.runtime.arrays:
            self.runtime.zeros("mobile_export_cumulative", size, dtype=wp.float64)
        if "mobile_flux_export_cumulative" not in self.runtime.arrays:
            self.runtime.zeros("mobile_flux_export_cumulative", size, dtype=wp.float64)
        if "avalanche_latch" not in self.runtime.arrays:
            self.runtime.zeros("avalanche_latch", size, dtype=wp.int32)
    ...
    def bind_device_state(
        ...
        Kernels consume the canonical ``mobile`` allocation directly so a
        checkpoint restore cannot leave a stale secondary alias behind.
        """
        ...
        # Always consume the authoritative field by its canonical name.
        # Device checkpoint restore may replace a Warp allocation; retaining
        # a historical ``height`` alias would then point at the pre-restore
        # allocation while Airborne writes the new ``mobile`` array.  That
        # split authority produced a zero-volume Mobile step with nonzero
        # landing momentum and immediate same-frame tail deposition.
        self.shape = grid.shape
        self.grid = grid
        self.material = material
        # FailureZone owns tool forcing in the shared Device state.  Refer to
        # its canonical allocation by name so checkpoint replacement cannot
        # leave another stale alias behind.
        self._forcing_array_name = "material_mask"
    ...
    def _summary(self, density: float) -> np.ndarray:
        wp = self.runtime.wp
        summary = wp.zeros(5, dtype=wp.float64, device=self.runtime.device)
    ...
    def step_resident(
        ...
        *,
        ...
        if tool_forcing_mask is not None:
            ...
            self.runtime.upload(self._forcing_array_name, forcing.ravel(), dtype=wp.int32)
        tool_velocity = (
        ...
        source_energy = wp.zeros(3, dtype=wp.float64, device=self.runtime.device)
        transport_crossings = wp.zeros(3, dtype=wp.float64, device=self.runtime.device)
        kernels = _kernels(wp)
        ...
        while remaining > 1.0e-14:
            ...
            self.runtime.launch(
                ...
                inputs=[
                    ...
                    self.runtime.arrays[self._forcing_array_name], self.runtime.arrays["weights"],
                    ...
                    gravity_impulse, friction_impulse, tool_impulse, source_energy,
                ],
            )
            self.runtime.launch(
                kernels[2], dim=size,
                inputs=[
                    self.runtime.arrays["mobile"], self.runtime.arrays["weights"],
                    self.runtime.arrays["velocity_x"], self.runtime.arrays["velocity_y"],
            ...
            self.runtime.launch(
                ...
                inputs=[
                    self.runtime.arrays["mobile"], self.runtime.arrays["velocity_x"],
        ...
        work = self._small(source_energy)
        crossings = self._small(transport_crossings)
        ...
        step = WarpMobileStep(
            ...
            kinetic_energy_before_j=float(before[4]),
            kinetic_energy_after_j=float(after[4]),
            gravity_pressure_work_j=float(work[0]),
            basal_friction_work_j=float(work[1]),
            tool_work_j=float(work[2]),
            transport_numerical_energy_residual_j=float(
                after[4] - before[4] - work[0] - work[1] - work[2]
            ),
            donor_export_m3=float(crossings[0]),
            receiver_import_m3=float(crossings[0]),
            # Every closed internal edge has one donor and one receiver.  The
            # independently reduced reservoir volume still supplies the
            # measured end-to-end residual instead of reporting a literal.
            transport_mass_residual_m3=float(after[0] - before[0]),
            advected_momentum_crossings_kg_m_s=np.asarray(
                [density * crossings[1], density * crossings[2], 0.0],
                dtype=np.float64,
            ),
        )
    ...
    def download_result(self) -> MobileLayerResult:
        ...
        height = self.runtime.download("mobile").reshape(self.shape)
        momentum = np.stack(
    ...
    def diagnostics(self) -> dict[str, object]:
        ...
        result.update({
            "resident_state": ["resting", "mobile", "momentum_x", "momentum_y", "weights"],

    ...
    def step(
        ...
        *,
        ...
        if window.empty:
            ...
            self._last_conservative_export_m3 = np.array(
                self._reference.last_conservative_export_m3, copy=True
            )
            return result
        ...
        self.last_total_time_ms = (perf_counter() - total_start) * 1_000.0
        self._last_conservative_export_m3 = np.zeros(grid.shape, dtype=np.float64)
        self._last_conservative_export_m3[slices] = (
            self._reference.last_conservative_export_m3
        )

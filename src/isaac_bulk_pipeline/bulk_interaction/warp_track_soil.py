...
def _kernels(wp: Any) -> Any:
    ...
    def sink_rut(
        ...
        if footprint[index] != 0:
            ...
            static_depth = wp.max(resting[index] - z_base[index], wp.float64(0.0))
            sinkage = wp.min(requested, wp.min(static_depth, available))
...
class WarpTrackSoilOperator:
    ...
    def initialize(
        ...
        self.runtime.zeros("z_base", self.size, dtype=wp.float64)
        self.runtime.arrays["initial_b_eff"] = self.runtime.arrays["initial_resting"]
        self.runtime.arrays["b_eff"] = self.runtime.arrays["resting"]
        self._initialize_workspace()
        self._initialized = True
    ...
    def _one_track_indices(
        ...
        self.runtime.launch(
            ...
            inputs=[
                ...
                self.runtime.arrays["initial_b_eff"],
                footprint_gpu,
    ...
    def download_state(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        resting = self.runtime.download("b_eff").reshape(self.shape)
    ...
    def diagnostics(self) -> dict[str, object]:
        ...
        result.update(
            {
                "parameter_status": self.config.parameter_status,
                "conservative_transfer": "b_eff_loss_equals_mobile_gain_donor_limited_by_b_eff_minus_z_base",
                "resident_state": ["z_base", "b_eff", "mobile", "momentum_x", "momentum_y"],
            }
        )
        return result
